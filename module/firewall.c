...
static int remove_connection_row(connection_rule_row *connection) {
    ...
    //     // kfree(connection);
    // spin_unlock(&klist_lock);
    connection->connection_rule_cli.state = STATE_INACTIVE;
    connection->connection_rule_srv.state = STATE_INACTIVE;
...
static int handle_fin_state( connection_rule_row* connection, connection_rule_t* rule, 
    ...
    char* terminator = (sender == 0) ? "srv": "cli";
        ...
        case STATE_FIN_WAIT_1:
            if (ack == ACK_YES && fin == FIN_NO && packet_sent) { // Simultaneous closing
                printk(KERN_INFO "STATCE_MACHINE_%s: Accepting for Wait_1 -> Closing", terminator);
                rule->state = STATE_CLOSING;
                print_connections_table();
                return NF_ACCEPT;
            } else if (ack == ACK_YES && fin == FIN_YES && !packet_sent) { // Simultaneous closing
                printk(KERN_INFO "STATCE_MACHINE_%s: Accepting for Wait_1 -> Closing", terminator);
                rule->state = STATE_CLOSING;
                print_connections_table();
                return NF_ACCEPT;
            } else if (ack == ACK_YES && fin == FIN_NO && !packet_sent) {
